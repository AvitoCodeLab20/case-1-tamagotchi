...
Go-сервис Avito Tamagotchi. Backend поднимает HTTP-сервер, подключается к PostgreSQL, предоставляет probes для оркестратора, реализует аутентификацию, недельный лидерборд и пользовательские награды, игровое API и обновление состояния питомца в реальном времени через WebSocket
...
| `GET` | `/api/v1/leaderboard/current` | Bearer | Текущий недельный рейтинг, место пользователя и прогресс до следующей призовой группы. |
| `GET` | `/api/v1/rewards` | Bearer | Инвентарь выданных наград и ожидающих выбора призов лидерборда. |
| `POST` | `/api/v1/rewards/{reward_id}/redeem` | Bearer | Активировать принадлежащую пользователю награду. |
| `POST` | `/api/v1/leaderboard/awards/{award_id}/select` | Bearer | Выбрать один из вариантов приза недельного лидерборда. |
